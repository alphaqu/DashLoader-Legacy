...
public class DashAndPredicate implements DashPredicate {
    ...
    public DashAndPredicate(AndMultipartModelSelector selector, StateManager<Block, BlockState> stateManager) {
        AndMultipartModelSelectorAccessor access = ((AndMultipartModelSelectorAccessor) selector);
        selectors = new ArrayList<>();
        access.getSelectors().forEach(selector1 -> selectors.add(PredicateHelper.getPredicate(selector1, stateManager)));
    }

    @Override
    public Predicate<BlockState> toUndash() {
        List<Predicate<BlockState>> list = selectors.stream().map(DashPredicate::toUndash).collect(Collectors.toList());
